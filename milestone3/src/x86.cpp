...
vector<instruction> codegen::make_x86_code(quad *q, int x, int y, int z){
    ...
    if(q->made_from == quad::BINARY){            // c(z) = a(x) op b(y)
        ...
        if(q->arg1[0]=='\'' or q->arg1[0]=='\"'){
        ins=instruction("leaq", str_map[q->arg1] +"(%rip)", "%rdi");
        insts.push_back(ins);}
        else{
        ...
        insts.push_back(ins);
         ...
         // b(y) = a(x)
        if(q->arg1[0]=='\"' or q->arg1[0]=='\''){
            ins= instruction("leaq", str_map[q->arg1]+"(%rip)", "%rax");
            insts.push_back(ins);
        ...
        if(y == 1) {    
            // cout<<"maind\n";    // make start label if it is the main function
            ins = instruction("", "main", "", "", "label");
            insts.push_back(ins);
        }

        else{ins = instruction("", get_func_name(q->arg1), "", "", "label");     // add label
        insts.push_back(ins);
        }
    ...
    // }
        ...
        //     insts.push_back(ins);
        // }
        if(this -> get_func_name(q->arg1) == "prints") {          // deal specially with print
            ins = instruction("add", "$8", "%rsp");
            insts.push_back(ins);
        }
...
void codegen::get_tac_subroutines() {
    ...
    for(auto q : root -> code) {
        // cout<<"here it "<<q->code<<endl;
...
void codegen::gen_global() {
    ...
    for(auto it:root->code){
        if (it->arg1[0]=='\"' or it->arg1[0]=='\''  ){
            ins=instruction("str"+to_string(str_cnt)+":",".string",it->arg1,"");
            this->code.push_back(ins);
            str_map[it->arg1] = "str"+to_string(str_cnt);
            str_cnt++;
        }
        if (it->arg2[0]=='\"' or it->arg2[0]=='\''  ){
            ins=instruction("str"+to_string(str_cnt)+":",".string",it->arg2,"");
            this->code.push_back(ins);
            str_map[it->arg2] = "str"+to_string(str_cnt);
            str_cnt++;
        }
    }
}
...
void codegen::print_code(string asm_file) {
    ...
    // ifstream printfs("printfs.s");
    // // string line;

    // while(getline(printfs, line)){
    //     out << line << '\n';
    // }
...
void subroutine_table::construct_subroutine_table(vector<quad*> subroutine_ins) {
    ...
    for(auto q : subroutine_ins) {
        if(q->made_from == quad::BEGIN_FUNC ||  q->made_from == quad::FUNC_CALL) {   // No nested procedures
            continue; 
        }
        ...
        if(q->made_from == quad::POP_PARAM) {
            ...
            if(q->made_from == quad::CONDITIONAL) {
                ...
                if(q->arg2 != "" && this -> lookup_table.find(q->arg2) == this -> lookup_table.end() && isVariable(q->arg2)) {
